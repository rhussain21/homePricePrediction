...
model1 <- lm(log(sale.price) ~ commercial.units, data = df)
model2 <- lm(log(sale.price) ~ commercial.units + log(sqft), data = df)
model3 <- lm(log(sale.price) ~ commercial.units + log(sqft) + age, data = df)
model4 <- lm(log(sale.price) ~ commercial.units + log(sqft) + age + median.income, data = df)
